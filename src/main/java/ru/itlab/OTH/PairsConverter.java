...
public class PairsConverter {
    ...
    public static void main(String[] s) {
        try {
            ...
            List<String> pairEUR_JPY = Files.readAllLines(Paths.get("USDCNY_220806_221106.csv"));
            List<String> pairEUR_RUB = Files.readAllLines(Paths.get("USDRUB_220806_221106.csv"));
            ...
            for (int i = 1; i < Math.min(pairEUR_JPY.size(), pairEUR_RUB.size()); i++) {
                ...
                pairJPY_RUB.add("CNYRUB;D;"+p_er[2] +";0;" + String.format("%.7f",open_ar).replaceAll(",", ".")+";"+
                        ";"+
                        ";"+
                        String.format("%.7f",close_ar).replaceAll(",", ".")+";");

                Files.write(Paths.get("pairs/CNYRUB_220806_221106.csv"),pairJPY_RUB);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
